...
import { handleHttpApi } from "../http/api.js";
...
const TOOLS: Tool[] = [
  ...
  },
  {
    name: "crawl_site",
    description:
      "Crawl a website from a start URL or sitemap, audit multiple same-origin pages, and return aggregate accessibility findings including per-page scores and the most repeated violation types.",
    inputSchema: zodToJsonSchema(crawlSiteSchema) as Tool["inputSchema"],
  },
  {
    name: "remediate",
    description:
      "End-to-end accessibility remediation in a single call. Audits a rendered URL with axe-core, then patches the corresponding source file. Three modes controlled by the 'mode' parameter: 'report' (audit only — no patching), 'diff' (audit + patch, returns before/after diff without writing to disk), 'fix' (audit + patch + write).",
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case "crawl_site": {
          const input = crawlSiteSchema.parse(args);
          const result = await crawlSite(input);
          return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        case "remediate": {
...
export async function startHttp(port: number): Promise<void> {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (await handleHttpApi(req, res, port)) {
      return;
    }
