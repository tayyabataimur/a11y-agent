...
function deriveWcagLevel(violations: Violation[]): WcagLevel {
  ...
  // Also check numeric WCAG tags from axe (wcag111, wcag143, etc.)
  const rawTags = new Set(violations.flatMap((v) =>
    (v as Violation & { _rawTags?: string[] })._rawTags ?? []
  ));

  if (violations.length === 0) return "AAA"; // perfect score
  if (hasA || rawTags.has("wcag2a")) return "Non-compliant";
  if (violations.some(v => v.wcag.some(t => /WCAG\s+\d+\.\d+\.\d+/.test(t)))) {
    // Has violations but none are Level A criteria — check if any are AA
    if (hasAA) return "A";
    return "AA";
  }
  return "Partial A";
...
// Violation-specific plain-English guidance with before/after examples
// derived from the actual failing HTML where possible
const VIOLATION_GUIDANCE: Record<string, {
  ...
  "html-has-lang": {
    headline: (_n) => "Page is missing a language declaration",
  ...
  "heading-order": {
    headline: (_n) => "Heading levels are out of order",
  ...
  "landmark-one-main": {
    headline: (_n) => "Page is missing a main landmark",
...
function generateSuggestion(
  ...
  const suggestion = guidance?.suggestion ??
    `Fix this violation by addressing the failing elements. See ${v.helpUrl} for detailed guidance.`;

  // If we have the actual failing HTML, show it in the before example
  let example_before = guidance?.example_before ?? (
    v.nodes[0]?.html ? `<!-- Failing element -->\n${v.nodes[0].html}` : `<!-- See failing elements below -->`
  );
  let example_after = guidance?.example_after ?? "<!-- See guidance at " + v.helpUrl + " -->";

  // Override with actual snippets when available and requested
  if (includeHtmlSnippets && v.nodes.length > 0 && v.nodes[0].html) {
    example_before = v.nodes
      .slice(0, 3)
      .map((n) => n.html)
      .join("\n");
  }
...
function buildAiSummary(result: Omit<EvaluateResult, "ai_summary">): string {
  ...
  lines.push(
    `Found ${summary.violations} violation${summary.violations > 1 ? "s" : ""} across ${summary.total_checks} checks: ` +
    `${summary.critical} critical, ${summary.serious} serious, ${summary.moderate} moderate, ${summary.minor} minor.`
  );
...
export async function evaluate(input: EvaluateInput): Promise<EvaluateResult> {
  const audit = await runAxeAudit(input.url);
  ...
  const top_issues = ranked.map((v, i) =>
    generateSuggestion(v, i + 1, input.include_html_snippets)
  );
  ...
  const ai_summary = buildAiSummary(partial);

  return { ...partial, ai_summary };
}
